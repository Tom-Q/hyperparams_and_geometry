"""Smoke test: MNIST dual-task. Run from repo root:
    python tests/mnist_dual.py
    python tests/mnist_dual.py --hidden-size 512 --depth 3
"""
import argparse, sys
...
final = train_network(
    task       = task,
    config     = config,
    run_dir    = Path("experiments_test") / "mnist_dual",
