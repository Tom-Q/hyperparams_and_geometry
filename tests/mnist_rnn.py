"""Smoke test: MNIST row-by-row (28 steps). Run from repo root:
    python tests/mnist_rnn.py
    python tests/mnist_rnn.py --cell-type rnn --hidden-size 256
"""
import argparse, sys
...
print(f"Sequence: {task.n_steps} steps × {task.input_size} features")
...
for cell_type in cell_types:
    ...
    final = train_network(
        task       = task,
        config     = config,
        run_dir    = Path("experiments_test") / f"mnist_rnn_{cell_type}",
        rdm_inputs = rdm_inputs,
        ds_train   = ds_train,
        ds_val     = ds_val,
        verbose    = True,
    )
    print(f"Best val_acc: {final:.4f}  ({'SOLVED' if final >= task.success_threshold else 'FAILED'})")
