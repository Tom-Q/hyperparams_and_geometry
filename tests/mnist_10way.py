"""Smoke test: MNIST 10-way. Run from repo root:
    python tests/mnist_10way.py
    python tests/mnist_10way.py --hidden-size 512 --depth 3
"""
import argparse, sys
...
final = train_network(
    task       = task,
    config     = config,
    run_dir    = Path("experiments_test") / "mnist_10way",
