"""Smoke test for spirals: trains one network and checks the recorded
...
# Spirals is hard for shallow MLPs; needs depth=3 and many epochs to reach
# val_acc >= 0.85 (success_threshold).
CONFIG = {
    "hidden_size":   128,
    "depth":         3,
    "batch_size":    64,
...
def test_train_and_activations(tmp_path):
    ...
    # A config that reaches val_acc >= 0.85 should cross most perf thresholds.
    assert len(perf_files) > 5
