"""Smoke test for mnist_rnn: trains one network and checks the recorded
...
# A config that should solve the task (val_acc >= 0.90) within a couple of epochs.
CONFIG = {
    "hidden_size":   64,
    "n_rnn_layers":  1,
    "batch_size":    64,
...
def test_train_and_activations(tmp_path):
    ...
    run_dir = tmp_path / "mnist_rnn_gru"
    train_network(
    ...
    # A config that reaches val_acc >= 0.90 should cross most perf thresholds.
